...
| API 23 签名 Release 安装、冷启动与基础 UI | artifact `8442770896` 的 x86_64 APK；`pm install -t`；`am force-stop`、`am start -W`、`logcat`、`uiautomator dump` | **通过（基础范围）**：安装成功，冷启动 512 ms、进程存活、Activity 位于前台、`AndroidRuntime` 无致命异常；内嵌 JS UI 层级已出现。未测试播放及外部入口 |
...
- 已发现风险：Provider authority 重复不阻止 API 23/35 安装和基础启动，但系统只把 authority 路由至项目 Provider，rn-fetch-blob 的 external path 因此不具备 owner；当前旧引擎的 Debug/Release 依赖图均为 1.8.0，目标 1.9.4 尚未证明可兼容。
